...
なおその前に、ビルダからの取得では、単体のオブジェクトが返る場合と、
コレクションで返る場合がありますが、コレクションの方はやりません。

一応Eloquentで取得する場合のコレクションは、
Eloquent\Collectionというもので、
通常のIlluminate\Support\Collectionを拡張したものではあるんですが、
Eloquentの全貌を理解するという視点においては
特別そこまで見るところがないんですよね……。
...
メソッド数は100を超えて、Eloquent\Modelから使われているトレイトの中では
最大です。Eloquent\Model本体でも130ほどなので。
...
実際簡単に分類できるものは、トレイトに移されて、その残りが本体にある、
という感じでしょう。
...
なぜか。get, set, is, has, あとas, fromみたいな、
同じような動詞で始まるメソッドばかり。
その内のどれが、あるいはそれ以外のどれが重要なのか、わからない。
...
理解した今から考えれば、名詞で使われているattribute, cast, mutatorが大事、
...
アクセサ・ミューテタ、キャストなどの、アトリビュートを変更する処理が、
アトリビュートの取得・設定をシンプルに
...
HasAttributesは、これらの処理を担っているトレイトなのです。
...
中心となるgetAttribute, setAttribute,
それらをラップするアクセサ・ミューテタ、キャスト関係の
メソッドの意図がわかりました。

この辺がHasAttributesの大半を占めています。
as, fromで始まるメソッドなんかもその関係です。
...
ほかのメソッドの名前を見ていくと、original, change(s), dirty
という名詞があります。

syncOriginalというメソッドが、
Modelから直接使われている数少ないメソッドの中に出てきたのを覚えていますか？
実はこの辺の機能は、行のライフサイクルに～。
...
ライフサイクルについて説明します。
HasAttributesにある3つのプロパティ、
$attributes, $original, $changes, さらに、syncOriginal, syncChanges,
この辺りが関係しているんですが、

$originalというのは、データベースにある原本のデータです。
実際にはデータベース内のデータは、ほかのプロセス等からいくらでも
変更されるわけですが、とりあえず行が生成された時点でデータベースに
あったデータ、と考えるとよいでしょう。
...
この辺に関係してくるものとしては、
isDirty, wasChangedというメソッドがあります。
この2つの違い、ライフサイクルを理解した上でならすぐわかります。
isDirtyは、現在のアトリビュートと、原本に差があるかを、
wasChangedは、$changes, つまり前回保存時に実際に変更されたものが
あるかを見ます。
...
Eloquentの全貌にだいぶ近付けてきている気がします。
さて、ここまでに見落しはなかったでしょうか？
...
ドキュメントのタイトルからざっと確認してみます。
Getting Startedには雑多な内容があります。大体見てきたと思いますが、
Prune関係は触れてないですね。割と最近追加された機能だったはずです。
Eloquentの全体像に関係する機能ではないので割愛です。

(めくる*3)
Relationshipsはざっと確認しました。Collectionsは飛ばしました。
Mutators / Castsもまあ大丈夫ですよね。

(めくる*2)
API Resourcesは、これはEloquentの機能ではないかな……。
次のSerializationはまあちゃんとEloquentの機能ですが、APIリソースは。
シリアライズは有用な機能ではありますが、Eloquentの全体像の中で
重要な機能ではないかなと思います。
visible/hiddenとの絡みあたりだけ抑えておけば大丈夫かと。
...
次はファイルから見てみましょう。
名前空間とファイルパスは対応しているので、該当個所を見れば、
一通りクラスも確認できるはずです。

直下のファイルは後回しにして、サブディレクトリから見ていきます。

Casts, Relationsはまあ問題なさそうです。Concernsは大体見ましたね。
Factoriesは見ていませんが、最初に言ったように、
ファクトリは今回対象外です。

ドキュメントでも、
Eloquentの方じゃなくてデータベースのシーディングのあたりに
書いてあるくらいですしね……。
